...
<div class="notes-layout">
    ...
    <!-- Right Column: Editor -->
    <div class="card" style="padding: 0; display: flex; flex-direction: column; height: 100%; border-radius: var(--radius-lg); border: 1px solid var(--border-color); background: var(--surface);">
        <!-- Editor Header -->
        <div style="padding: 1.25rem; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
            <div style="flex: 1; min-width: 250px;">
                <input type="text" form="editNoteForm" name="title" value="<?= htmlspecialchars($note['title']) ?>" placeholder="Judul Catatan..." required style="font-size: 1.25rem; font-weight: 700; color: var(--text-main); background: transparent; border: none; outline: none; width: 100%; margin-bottom: 0.5rem; padding: 0;">
                <select name="course_id" form="editNoteForm" required style="display: inline-flex; align-items: center; gap: 0.5rem; background: rgba(99,102,241,0.1); color: var(--primary); padding: 0.25rem 0.75rem; border-radius: var(--radius-full); font-size: 0.8rem; font-weight: 600; border: none; outline: none; cursor: pointer;">
                    <option value="" disabled>Pilih Mata Kuliah...</option>
                    <?php foreach($courses as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= $c['id'] == $note['course_id'] ? 'selected' : '' ?>><?= htmlspecialchars($c['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div style="display: flex; gap: 0.5rem;">
                <a href="<?= BASE_URL ?>/notes/export_pdf?id=<?= $note['id'] ?>" class="btn btn-outline" style="color: #4b5563; border-color: #d1d5db;" target="_blank">
                    <i class="fa-solid fa-file-pdf"></i> Export PDF
                </a>
                <a href="javascript:void(0)" class="btn btn-outline" style="color: var(--accent-rose); border-color: var(--accent-rose);" onclick="confirmDelete('<?= BASE_URL ?>/notes/delete?id=<?= $note['id'] ?>', 'Hapus catatan ini?');">
                    <i class="fa-solid fa-trash"></i>
                </a>
                <button type="submit" form="editNoteForm" class="btn btn-primary">
        ...
        <!-- Editor Content -->
        <form id="editNoteForm" action="<?= BASE_URL ?>/notes/update" method="POST" style="flex: 1; display: flex; flex-direction: column; position: relative;" onsubmit="document.getElementById('hiddenContent').value = quill.root.innerHTML">
    ...
    function generateNoteAi() {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // Clear content or append? Let's just append with a new line
                let currentLength = quill.getLength();
                if (currentLength > 1) {
                    originalHtml = quill.root.innerHTML + "<br><br><strong>--- Catatan AI ---</strong><br><br>";
                } else {
                    originalHtml = '';
                }
                currentHtml = '';
